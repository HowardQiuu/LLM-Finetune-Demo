...
# 3. build vector store/FAISS index
...
# 6. RAG query function with rerank
def rag_query(query, top_k=3, rerank_k=2):
    # (1) query -> embedding
    ...
    reranked_docs = [doc for _, doc in sorted(zip(rerank_scores, retrieved_docs), reverse=True)]
    retrieved_docs = reranked_docs[:rerank_k]
    # (4) construct prompt
    context = "\n".join(retrieved_docs)
